...
export function Composer({ running, disabled, send, stop, models, model, effort, permission, threadUsage, compaction, onModel, onEffort, onPermission }: { running: boolean; disabled: boolean; send: (text: string, attachments: ComposerAttachment[]) => boolean; stop: () => void; models: ModelOption[]; model: string; effort: string; permission: PermissionPreset; threadUsage: ThreadUsage | null; compaction: CompactionStatus; onModel: (model: string) => void; onEffort: (effort: string) => void; onPermission: (permission: PermissionPreset) => void }) {
  ...
  const submit = () => { if ((!text.trim() && !attachments.length) || running || disabled) return; if (send(text.trim(), attachments)) { setText(""); setAttachments([]); setAttachmentError(""); } };
  ...
  return <><div className="composer-wrap"><div className="composer">
    ...
    <textarea ref={area} value={text} disabled={disabled || running} rows={1} placeholder={disabled ? "Connect to Codex to start…" : running ? "Codex is working…" : "Ask Codex to work on your code"} onPaste={paste} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && matchMedia("(min-width: 701px)").matches) { event.preventDefault(); submit(); } }} />
    <div className="composer-toolbar"><div className="composer-tools"><input ref={input} className="attachment-input" type="file" multiple disabled={disabled || running} onChange={(event) => addFiles(event.target.files)} /><button className="attach-button" type="button" disabled={disabled || running} aria-label="Add attachment" onClick={() => input.current?.click()}><Paperclip /></button><ModelPicker models={models} model={model} effort={effort} onModel={onModel} onEffort={onEffort} /><PermissionPicker permission={permission} disabled={running} onPermission={onPermission} />{effort && <span className="effort-chip">{effort}</span>}</div>
